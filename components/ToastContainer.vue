<script setup>
/**
 * Toast notification container — renders above the tabs of the popup.
...
<template>
  <div role="status" aria-live="polite" aria-atomic="false" class="fixed bottom-14 left-2 right-2 z-50 space-y-1.5 pointer-events-none">
    <TransitionGroup
    ...
    >
      ...
      >
        ...
        <button :aria-label="translate('common.close')" @click="dismiss(t.id)" class="shrink-0 p-0.5 opacity-60 hover:opacity-100 transition-opacity min-w-8 min-h-8">
          <X class="w-3 h-3" />
        </button>
      </div>
    </TransitionGroup>
