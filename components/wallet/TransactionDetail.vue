  ...
  <div class="animate-fade-in-up" :class="fullPage ? 'max-w-2xl mx-auto' : ''">
    <div class="flex items-center justify-between mb-4">
      ...
      <div class="flex items-center gap-1">
        <button @click="shareReceipt" class="p-1.5 rounded-lg hover:bg-surface-elevated text-text-muted min-w-8 min-h-8" :title="t('wallet.shareReceipt')">
        ...
        <button v-if="hasTechnicalData" @click="showTechnical = !showTechnical"
          class="p-1.5 rounded-lg hover:bg-surface-elevated min-w-8 min-h-8" :class="showTechnical ? 'text-brand' : 'text-text-muted'"
          :title="t('wallet.technicalDetails')"><Code class="w-4 h-4" /></button>
        <button @click="emit('close')" class="p-1.5 rounded-lg hover:bg-surface-elevated min-w-8 min-h-8" :aria-label="t('common.close')">
    ...
    <div class="bg-surface-card rounded-3xl border border-border p-5 mb-4 shadow-md text-center">
      ...
      <span class="inline-flex items-center gap-1 mt-1 px-2 py-0.5 rounded-full text-xs font-semibold" :class="statusColor">
    ...
    <a v-if="metadata.merchantVerification?.verifyUrl" :href="metadata.merchantVerification.verifyUrl" target="_blank" rel="noopener noreferrer"
      ...
      <div class="min-w-0 flex-1"><p class="text-xs font-bold">{{ t('wallet.verifiedByBranta') }}</p><p class="text-xs text-text-muted truncate">{{ metadata.merchantVerification.name }}</p></div>
    ...
    <div v-if="metadata.source === 'mobile' || metadata.payout || metadata.deliveryStatus?.hasPayout" class="rounded-2xl border p-4 mb-3"
      ...
      <button v-if="metadata.verifyUrl && !checkingDelivery && !metadata.deliveryStatus?.delivered" @click="refreshDelivery"
        class="mt-2 text-xs font-semibold text-info hover:underline">{{ t('wallet.checkDelivery') }}</button>
    </div>

    <!-- LUD-09 success action -->
    <div v-if="metadata.successAction" class="bg-surface-card rounded-2xl border border-border p-4 mb-3">
      <p class="text-xs uppercase tracking-wider font-semibold text-text-muted mb-1.5">{{ t('wallet.messageFromService') }}</p>
    ...
    <div class="bg-surface-card rounded-3xl border border-border divide-y divide-border overflow-hidden mb-3 shadow-sm">
      <div class="px-4 py-3">
        <div class="text-xs text-text-muted font-semibold uppercase tracking-wider mb-0.5">{{ t('wallet.description') }}</div>
      ...
      <div v-if="metadata.recipientAddress" class="px-4 py-3">
        <div class="text-xs text-text-muted font-semibold uppercase tracking-wider mb-0.5">{{ t('wallet.recipient') }}</div>
        <p class="text-xs font-mono break-all">{{ metadata.recipientAddress }}</p>
      </div>
      <div class="grid grid-cols-2 divide-x divide-border">
        <div class="px-4 py-3"><div class="text-xs text-text-muted font-semibold uppercase tracking-wider mb-0.5">{{ t('wallet.created') }}</div><p class="text-xs">{{ createdDate || '—' }}</p></div>
        <div class="px-4 py-3"><div class="text-xs text-text-muted font-semibold uppercase tracking-wider mb-0.5">{{ t('wallet.settled') }}</div><p class="text-xs">{{ settledDate || '—' }}</p></div>
      </div>
      <div class="grid grid-cols-2 divide-x divide-border">
        <div class="px-4 py-3"><div class="text-xs text-text-muted font-semibold uppercase tracking-wider mb-0.5">{{ t('wallet.fees') }}</div><p class="text-xs">{{ feesPaid ? `${formatSats(feesPaid)} ${t('wallet.sats')}` : t('wallet.feesNone') }} <span v-if="fiatFees" class="text-text-muted">({{ fiatFees }})</span></p></div>
        <div class="px-4 py-3"><div class="text-xs text-text-muted font-semibold uppercase tracking-wider mb-0.5">{{ t('wallet.total') }}</div><p class="text-xs font-semibold">{{ formatSats(totalPaid) }} {{ t('wallet.sats') }}</p></div>
      </div>
    </div>

    <!-- Personal note -->
    <div class="bg-surface-card rounded-2xl border border-border p-4 mb-3">
      <div class="flex items-center justify-between mb-2"><label class="text-xs uppercase tracking-wider font-semibold text-text-muted">{{ t('wallet.personalNote') }}</label><span class="text-xs text-text-muted">{{ personalNote.length }}/500</span></div>
      <textarea v-model="personalNote" :aria-label="t('wallet.personalNote')" maxlength="500" rows="2" :placeholder="t('wallet.personalNotePlaceholder')"
      ...
      <button @click="saveNote" :disabled="savingNote || !transactionId" class="mt-2 ml-auto flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-brand/10 text-brand text-xs font-semibold disabled:opacity-40">
    ...
    <div v-if="hasTechnicalData && showTechnical" class="bg-surface-card rounded-3xl border border-border divide-y divide-border overflow-hidden animate-fade-in-up shadow-sm">
      <div class="px-4 py-2 bg-surface-elevated/50 flex items-center gap-1.5"><Code class="w-3 h-3 text-text-muted" /><span class="text-xs text-text-muted font-semibold uppercase tracking-wider">{{ t('wallet.technicalDetails') }}</span></div>
      <div v-for="field in [
        ...
        { id: 'verify', label: t('wallet.checkDelivery'), value: metadata.verifyUrl },
      ].filter(item => item.value)" :key="field.id" class="px-4 py-3">
        <div class="flex items-center justify-between mb-1"><span class="text-xs text-text-muted font-semibold uppercase tracking-wider">{{ field.label }}</span><button @click="copy(field.value, field.id)" class="p-0.5 rounded hover:bg-surface-elevated min-w-8 min-h-8"><Check v-if="copiedField === field.id" class="w-3 h-3 text-success" /><Copy v-else class="w-3 h-3 text-text-muted" /></button></div>
        <code class="text-xs font-mono text-text-secondary break-all leading-relaxed">{{ field.value }}</code>
      </div>
    </div>
  </div>
</template>
