...
import { Lock, Eye, EyeOff, AlertTriangle, KeyRound, ShieldCheck, ChevronDown, Globe } from 'lucide-vue-next'
  ...
  <div class="flex flex-col items-center justify-center min-h-[460px] px-6 py-8">
    ...
    <div class="animate-scale-in mb-6">
      <div class="relative">
        <div class="w-16 h-16 rounded-2xl bg-brand/10 flex items-center justify-center">
          <Lock v-if="!isSetup" class="w-7 h-7 text-brand" />
          <KeyRound v-else class="w-7 h-7 text-brand" />
        </div>
    ...
    <div v-if="isSetup" class="relative mb-4 animate-fade-in-up" ref="langDropdownRef">
      <button
        ...
        class="flex items-center gap-1.5 px-2.5 py-1.5 rounded-lg border border-border hover:border-brand/40 transition-colors text-xs text-text-secondary"
      >
      ...
      <div
        ...
        class="absolute left-1/2 -translate-x-1/2 top-full mt-1 w-44 max-h-52 overflow-y-auto bg-surface-card rounded-xl border border-border shadow-lg z-50 py-1 animate-scale-in origin-top"
      >
        <button
          v-for="lang in locales"
          :key="lang.code"
          @click="pickLang(lang.code)"
          class="w-full flex items-center justify-between px-3 py-2 text-xs hover:bg-surface-elevated transition-colors text-left"
    ...
    <div class="w-full space-y-3 animate-fade-in-up stagger-2" @keydown="handleKeydown">
      ...
      <div class="space-y-1.5">
        ...
        <div class="relative">
          ...
          <button
            ...
            class="absolute right-2.5 top-1/2 -translate-y-1/2 p-1 text-text-muted hover:text-text-secondary transition-colors"
            tabindex="-1"
          >
      ...
      <button
        @click="submit"
        :disabled="!canSubmit"
        class="w-full py-2.5 text-sm rounded-xl bg-brand text-surface-base hover:bg-brand-hover disabled:opacity-40 disabled:cursor-not-allowed transition-colors font-semibold btn-primary"
      >
