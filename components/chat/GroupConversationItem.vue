...
const preview = computed(() => {
  ...
  const senderName = senderProfile?.display_name || senderProfile?.name || props.lastMessage.sender?.slice(0, 8) + '...'
  ...
  >
    ...
    <div class="relative">
      <div
        ...
        :style="!group.picture ? { background: avatarColor } : {}"
      >
        <img v-if="group.picture" :src="group.picture" alt="" class="w-full h-full object-cover" @error="group.picture = null" />
      ...
      <div class="absolute -bottom-0.5 -right-0.5 w-5 h-5 rounded-full bg-surface-card border border-border flex items-center justify-center">
        <Users class="w-2.5 h-2.5 text-text-muted" />
      </div>
    </div>
