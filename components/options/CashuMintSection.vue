...
const { t } = useI18n()
const { wallets, getCashuMintInfo } = useWallet()
...
async function fetchMintInfo() {
  ...
  try {
    let info = null
    try { info = await getCashuMintInfo(target) } catch { /* try fallback */ }
    if (!info) {
      const res = await fetch(target.replace(/\/+$/, '') + '/v1/info')
...
async function validateNewMint() {
  ...
  try {
    let info = null
    try { info = await getCashuMintInfo(url) } catch { /* try fallback */ }
    if (!info) {
...
<template>
  <div class="space-y-4">
    ...
    <!-- Mint info tooltip -->
    <div class="relative inline-flex group/mint">
      <button class="flex items-center gap-1.5 text-[11px] text-text-muted hover:text-brand transition-colors duration-200">
        <Info class="w-3.5 h-3.5" />
        <span class="font-medium">{{ t('cashu.whatsAMint') }}</span>
      </button>
      <!-- Tooltip -->
      <div class="pointer-events-none absolute left-0 top-full mt-2 w-[340px] opacity-0 group-hover/mint:opacity-100 group-hover/mint:pointer-events-auto transition-all duration-200 translate-y-1 group-hover/mint:translate-y-0 z-50">
