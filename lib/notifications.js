...
async function notificationText() {
  let messages = en.notificationMessages
  let locale = 'en'
  try {
    const saved = await chrome.storage.local.get('locale')
    locale = saved.locale || 'en'
    if (locale !== 'en') messages = (await import(`../locales/${locale}.json`)).default.notificationMessages
  } catch { locale = 'en' }
  return (key, values = {}) => (messages[key] || en.notificationMessages[key]).replace(/\{(\w+)\}/g, (_, name) => typeof values[name] === 'number' ? new Intl.NumberFormat(locale).format(values[name]) : String(values[name] ?? ''))
}
...
export async function notifyDm(senderName, preview, messageId) {
  ...
  const t = await notificationText()
  const title = senderName || t('newMessage')
  const message = preview || t('messageBody')
...
export async function notifyPayment(amountSats, paymentHash) {
  ...
  await createNotification(`payment-${paymentHash || Date.now()}`, {
    ...
    message: t('paymentBody', { amount: amountSats }),
  })
}
...
export async function notifyBudgetSpend(host, amountSats, remainingSats) {
  ...
  await createNotification(`budget-${host}-${Date.now()}`, {
    title: t('budgetTitle', { amount: amountSats }),
    message: t('budgetBody', { remaining: remainingSats, site: host }),
  })
}
