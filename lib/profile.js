 ...
 * Uses nostr-core for event creation and metadata parsing.
 */

import { finalizeEvent, hexToBytes, nip24, addAltTag } from 'nostr-core'
 ...
 * @param {Object} profile - { name, about, picture, nip05, display_name, ... }
...
export async function publishProfile(profile, secretHex, relays = DEFAULT_ACCOUNT_RELAYS) {
  // Filter out empty values, build content via nostr-core
  const metadata = {}
  for (const [key, value] of Object.entries(profile)) {
    if (value) metadata[key] = value
  }
  const content = nip24.buildMetadataContent(metadata)

  const secretKey = hexToBytes(secretHex)
  const tags = []
  addAltTag(tags, `Profile metadata for ${metadata.name || metadata.display_name || 'user'}`)
...
export async function fetchProfile(pubkey, relays = DEFAULT_ACCOUNT_RELAYS) {
  ...
  try {
    ...
    return nip24.parseExtendedMetadata(latest)
  } catch {
    return null
  }
}
