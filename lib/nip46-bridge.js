 ...
 *
 * nostr-core v0.6.0 provides the full NIP-46 client (NostrConnect class).
 * This file only adds:
...
import {
  NostrConnect, parseConnectionURI as parseCoreConnectionURI,
...
export function createNostrConnectURI({ clientPubkey, relayUrl, secret, name, url, image }) {
  if (!/^[0-9a-f]{64}$/i.test(clientPubkey || '')) throw new Error('NIP-46 client public key is invalid')
  relayUrl = requireSecureRelay(relayUrl)
...
function requireSecureRelay(relayUrl) {
  let relay
  try { relay = new URL(relayUrl) } catch { throw new Error('NIP-46 relay URL is invalid') }
  if (relay.protocol !== 'wss:' || relay.username || relay.password) {
    throw new Error('NIP-46 requires secure WSS relays')
  }
  return relay.toString().replace(/\/$/, '')
}

export function parseConnectionURI(uri) {
  if (typeof uri !== 'string' || uri.length > 16_384) throw new Error('NIP-46 connection URI is invalid')
  const parsed = parseCoreConnectionURI(uri)
  if (!/^[0-9a-f]{64}$/i.test(parsed.remotePubkey || '')) throw new Error('NIP-46 signer public key is invalid')
  if (!Array.isArray(parsed.relayUrls) || parsed.relayUrls.length === 0 || parsed.relayUrls.length > 10) {
    throw new Error('NIP-46 relay list is invalid')
  }
  if (parsed.secret && (typeof parsed.secret !== 'string' || parsed.secret.length > 512)) {
    throw new Error('NIP-46 connection secret is invalid')
  }
  return { ...parsed, relayUrls: [...new Set(parsed.relayUrls.map(requireSecureRelay))] }
}
