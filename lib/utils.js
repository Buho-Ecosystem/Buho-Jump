...
export function detectPaymentInput(input) {
  ...
  if (isSARetailerQR(s)) {
    ...
    if (isConvertibleQR(s)) {
      // Phase 1: EMVCo QR → convertible to Lightning Address via cryptoqr.net
      const lnAddress = convertToLightningAddress(s)
      return { type: 'merchant', value: lnAddress, merchant, qrPayload: s }
    }
    // Phase 2: recognized but not yet supported (URL-based QR codes)
  ...
  // LUD-17: protocol-specific URL schemes
  if (lower.startsWith('lnurlp://')) {
    return { type: 'lnurl', value: s, lnurlType: 'pay' }
  }
  if (lower.startsWith('lnurlw://')) {
    return { type: 'lnurl', value: s, lnurlType: 'withdraw' }
  }
  if (lower.startsWith('keyauth://')) {
    return { type: 'lnurl', value: s, lnurlType: 'auth' }
  }
  // Legacy bech32-encoded LNURL
  if (lower.startsWith('lnurl1')) {
    return { type: 'lnurl', value: lower }
  }
