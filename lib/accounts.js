...
/**
 * Create a new local account with a mnemonic (recovery words).
 * Returns account data + mnemonic for one-time backup display.
 */
export async function createAccountWithMnemonic(name) {
...
/**
 * Import an account from a 12- or 24-word mnemonic (recovery words).
 */
export async function importFromMnemonic(name, mnemonic) {
  ...
  const account = {
    id,
    name: name || 'Recovered ' + id.slice(0, 4),
    pubkey,
    secretHex,
    mode: 'local',
    nip46Session: null,
