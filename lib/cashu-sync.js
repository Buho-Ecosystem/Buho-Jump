...
export async function publishWalletEvent(secretKey, walletPrivkey, mints, pubkey) {
  try {
    ...
    await Promise.allSettled(relays.map(url =>
      pool.publish([url], event).catch(() => {})
    ))
    log.info('cashu-sync', 'WALLET_PUBLISHED', { mints: mints.length })
  } catch (err) {
...
export async function publishTokenEvent(secretKey, mintUrl, proofs, pubkey, delEventIds) {
  try {
    ...
    log.info('cashu-sync', 'TOKEN_PUBLISHED', { proofs: proofs.length })
    return event.id
  } catch (err) {
...
export async function publishHistoryEvent(secretKey, direction, amountSats, tokenEventIds, pubkey) {
  try {
    ...
    log.info('cashu-sync', 'HISTORY_PUBLISHED', { direction, amount: amountSats })
  } catch (err) {
