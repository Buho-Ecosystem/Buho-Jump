 ...
 * using AES-256-GCM via lib/crypto.js.
 */
...
async function readStore(password) {
  ...
  if (data[STORAGE_KEY]?.encrypted) {
    try {
      return await decryptData(data[STORAGE_KEY].encrypted, password)
    } catch {
      return emptyStore()
    }
  }
