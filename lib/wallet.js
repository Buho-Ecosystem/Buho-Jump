...
export async function addWallet(connectionUri, name, password) {
  ...
  try {
    ...
    if (!parsed.hostname || parsed.hostname.length < 64) throw new Error('Missing wallet key')
    if (!parsed.searchParams.get('secret')) throw new Error('Missing secret')
    if (!parsed.searchParams.getAll('relay').some(r => r.startsWith('wss://'))) throw new Error('Missing relay')
  } catch (e) {
    throw new Error('Invalid wallet connection string')
  }
