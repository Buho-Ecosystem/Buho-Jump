...
export function createRequestCoordinator({
  ...
  function attachWindowClose(requestId, win, fallbackValue = false) {
    if (!supportsWindowsApi || !win?.id || !windowsApi?.onRemoved?.addListener) return
    const entry = pending.get(requestId)
    if (!entry) return

    const onRemoved = (windowId) => {
      if (windowId !== win.id || !pending.has(requestId)) return
      resolve(requestId, fallbackValue).catch(() => {})
    }

    windowsApi.onRemoved.addListener(onRemoved)
    entry.cleanupFns.push(() => windowsApi.onRemoved.removeListener(onRemoved))
  }
